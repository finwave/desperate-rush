...
HRESULT CSoundWave::Create(	CSoundEngine* pSoundEngine,
							DWORD dwFrequency, 
							...
							DWORD dwFlags)
{
	...
	LPDIRECTSOUND8	pDS = pSoundEngine->GetDirectSound();
...
// Load
// load wave file into the buffer and create the duplicates
HRESULT CSoundWave::Load(	CSoundEngine* pSoundEngine,
							LPCTSTR strFilename,
							DWORD dwDuplicates,
							DWORD dwFlags)
{
...
void CSoundWave::Play(BOOL bLoop, DWORD dwDuplicate, int iVolume)
{
	DWORD dwFlags = 0;
	if(bLoop)
	{
